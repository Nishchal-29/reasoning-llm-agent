...
# LoRA training for encoder-decoder LM (Flan-T5) to map prompt -> short label (target).
# Training format: each example is {"input": prompt_str, "target": target_str}
...
def preprocess_batch(examples, tokenizer, max_source_length, max_target_length):
...
def main():
    ...
    n_total = sum(p.numel() for p in model.parameters())
    print("Trainable params:", n_trainable, "Total params:", n_total)
    print("Tokenizing dataset without padding (collator will pad)...")
    ds_tokenized = ds.map(
        lambda ex: preprocess_batch(ex, tokenizer, args.max_source_length, args.max_target_length),
        batched=True,
